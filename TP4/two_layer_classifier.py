...
class TwoLayerClassifier(object):
    ...
    def predict(self, x):
        ...
        if len(x.shape) == 1:  # Predict on one sample
            # the most probable class label for one sample
            srt = self.net.forward(x)
            srt = np.exp(srt)
            # srt2 = -np.exp(srt)
            srt = srt / np.sum(srt)
            classe_proba = np.argmax(srt)
            return classe_proba

        elif len(x.shape) == 2:  # Predict on multiple samples
            # the most probable class label for many samples 
            srt = self.net.forward(x)
            srt = np.exp(srt)
            denom = np.sum(srt, axis=1).reshape((-1, 1))
            srt = srt / denom
            classe_proba = np.argmax(srt, axis=1)
    ...
    def global_accuracy_and_cross_entropy_loss(self, x, y, l2_r=-1.0):
        ...
        loss_indiv = np.sum((- t * np.log(srt)),axis=1)
        loss=np.mean(loss_indiv)
        if l2_r > 0:
            for param in self.net.parameters:
                loss = loss+np.sum(( self.net.l2_reg * (np.linalg.norm(param))**2))
...
class TwoLayerNet(object):
    ...
    def cross_entropy_loss(self, scores, y):
        ...
        somme=np.sum(srt)
        srt = srt/somme
        # Compute cross-entropy loss and regularization
        C = self.num_classes

        t = np.zeros(C)
        t[y] = 1
        loss = -(np.sum( np.multiply(t,np.log(srt))))
        for param in self.parameters:
            loss =loss + np.sum(self.l2_reg * np.linalg.norm(param))
